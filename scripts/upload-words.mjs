...
async function runDryRun(docs) {
  console.log(
    `[dry-run] ${docs.length} valid docs ready to seed (no Firestore calls made).`,
  );
  console.log("[dry-run] Sample doc:");
  console.log(
    JSON.stringify({ id: toDocId(docs[0].word), ...docs[0] }, null, 2),
  );
  ...
  console.log(
    `[dry-run] Unique doc IDs: ${ids.size} (${collisions} collisions among input words)`,
  );
}
...
async function main() {
  ...
  console.log(
    `Loaded ${dataset.length} entries from ${DATA_FILE}, ${docs.length} valid.`,
  );
